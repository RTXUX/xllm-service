...
## Request Dispatch
Prefill replica:
- Pull batch from Queue and run prefill and push to MigrationQueue
- FCFS, work stealing
MigrationQueue:
- Try each decode replica (first-fit by capacity)
Decode replica:
- Receive migrated batch and run decode loop


## EVERY CONTROL LOOP ITERATION

  1. SCAN all replicas → categorize into:
     {Inactive, Prefill, Decode, Shutting*, Loading*, Zigzag*}

  2. COMPUTE constraints:
     a) Eq.1: min_decode_replicas based on memory pressure
     b) Eq.2: min_decode_replicas based on KV-cache bandwidth
     c) Eq.3: min_prefill_replicas based on prefill throughput demand

  3. GENERATE SCALE PLAN:
     delta_prefill = clamp(target - current, -1, +∞)
     delta_decode  = clamp(target - current, -1, +∞)
     // Always keep ≥1 of each

  4. EXECUTE PLAN (sorted by used_blocks ascending):
     IF scale_up:
       Pick Inactive replica → load model via best mechanism:
         - NVLink broadcast (same machine, fastest)
         - RDMA P2P (cross-machine)
         - RDMA broadcast chain
         - Host cache / SSD (ServerlessLLM fallback)
     IF scale_down:
       Pick least-loaded active replica → mark ShuttingPrefill/ShuttingDecode
       Wait for in-flight work to drain → Inactive
...
## Map to the current framework
- Scale up: find an idle instance and wake up it
- Scale down: cordon the selected instance, put the selected instance into sleep, maybe wait its work to drain
- No zigzag
- Map only the policy part, the execution part is unnecessary, as the loading is already fast enough in current framework